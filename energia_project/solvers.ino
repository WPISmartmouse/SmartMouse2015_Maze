...
void flood_explore(Maze *kmaze){
  ...
  char *no_wall_path;
  no_wall_path = (char *)malloc(PATH_SIZE*sizeof(char));
  char *all_wall_path;
  all_wall_path = (char *)malloc(PATH_SIZE*sizeof(char));
  ...
  do {
    ...
    for (i=0;i<4;i++){
      int r = mouse->row;
      int c = mouse->col;
      update_pos((Direction)i, &r, &c);
      Node *all_wall_node_neighbor = get_node(all_wall_maze,r,c);

      update_nodes(walls,(Direction)i,no_wall_node,all_wall_node,all_wall_node_neighbor);
    }
...
void left_hand_follow(Maze *maze){
  ...
  while (!atCenter(mouse)){
    Direction dir = (Direction)((mouse->dir-1)%4); //check left first
